...
void generateInlineGetters()
{
  ...
  for(size_t i=0; i<derivatives.size(); i++)
  {
    ...
    //      virtual void get(signed char       & iValue) { setSInt8 ( static_cast<Variant::sint8> (iValue)); }
    printf("    virtual void get(%s & iValue) const { iValue = get%s(); }\n", 
...
void generateCompareDefinitions()
{
  ...
  printf("  {\n");
  printf("    //force boolean to be compared as sint8 to prevent undefined behavior\n");
  printf("    if (isBoolean(iLocalValue))\n");
  printf("      return compareNativeTypes( static_cast<Variant::sint8>(iLocalValue), iRemoteValue );\n");
  printf("    if (isBoolean(iRemoteValue))\n");
  printf("      return compareNativeTypes( iLocalValue, static_cast<Variant::sint8>(iRemoteValue) );\n");
  printf("\n");
  ...
  printf("  }\n");
  printf("\n");
  printf("  inline int compareStrings(const Variant::Str & iLocalValue, const Variant::Str & iRemoteValue)\n");
  printf("  {\n");
  printf("    if (iLocalValue < iRemoteValue)\n");
  printf("    {\n");
  printf("      return -1;\n");
  ...
  printf("      oResult = compareNativeTypes(iData.vbool, iValue);\n");
  printf("      return true;\n");
  printf("    case Variant::VariantFormat::UINT8:\n");
  printf("      oResult = compareNativeTypes(iData.vuint8, iValue);\n");
  printf("      return true;\n");
  printf("    case Variant::VariantFormat::UINT16:\n");
  printf("      oResult = compareNativeTypes(iData.vuint16, iValue);\n");
  printf("      return true;\n");
  printf("    case Variant::VariantFormat::UINT32:\n");
  printf("      oResult = compareNativeTypes(iData.vuint32, iValue);\n");
  printf("      return true;\n");
  printf("    case Variant::VariantFormat::UINT64:\n");
  printf("      oResult = compareNativeTypes(iData.vuint64, iValue);\n");
  printf("      return true;\n");
  printf("    case Variant::VariantFormat::SINT8:\n");
  printf("      oResult = compareNativeTypes(iData.vsint8, iValue);\n");
  printf("      return true;\n");
  printf("    case Variant::VariantFormat::SINT16:\n");
  printf("      oResult = compareNativeTypes(iData.vsint16, iValue);\n");
  printf("      return true;\n");
  printf("    case Variant::VariantFormat::SINT32:\n");
  printf("      oResult = compareNativeTypes(iData.vsint32, iValue);\n");
  printf("      return true;\n");
  printf("    case Variant::VariantFormat::SINT64:\n");
  printf("      oResult = compareNativeTypes(iData.vsint64, iValue);\n");
  printf("      return true;\n");
  printf("    case Variant::VariantFormat::FLOAT32:\n");
  printf("      oResult = compareNativeTypes(iData.vfloat32, iValue);\n");
  printf("      return true;\n");
  printf("    case Variant::VariantFormat::FLOAT64:\n");
  printf("      oResult = compareNativeTypes(iData.vfloat64, iValue);\n");
  printf("      return true;\n");
  ...
  for(size_t i=0; i<numInnerTypes; i++)
  {
    const INNER_TYPE & _type = getInnerType(i);
    {
      ...
      printf("    return compareStrings( (*mData.str), Variant(iValue).getString() );\n");
      printf("  }\n");
      printf("  \n");
    }
  }
...
void generateOperator(char iOperator)
{
  ...
  for(size_t i=0; i<derivatives.size(); i++)
  {
    ...
    //      virtual Variant operator + (const unsigned long     & iValue) const { Variant tmpCopy(*this); tmpCopy += static_cast<Variant::uint32> (iValue); return tmpCopy; }
...
void generateOperatorOrEquals(char iOperator)
{
  ...
  for(size_t i=0; i<derivatives.size(); i++)
  {
    ...
    //      virtual const Variant & operator += (const unsigned long     & iValue) { return ( (*this) += static_cast<Variant::uint32> (iValue) ); }
