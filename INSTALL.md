...
1) Download the source code from an existing [tags](http://github.com/end2endzone/libVariant/tags) and extract the content to a local directory (for example `c:\projects\libVariant` or `~/dev/libVariant`).
...
These are the base requirements to build and use libVariant:
...
1) Get a copy of the source code by using one of the following methods:
   * Download the source code of the project from an existing [tags](http://github.com/end2endzone/libVariant) and extract the downloaded zip file to a local directory (for example `c:\projects\libVariant` or `~/dev/libVariant`).
   * Clone the github repository by running `git clone "http://github.com/end2endzone/libVariant"`.
...
# Testing #
libVariant comes with unit tests which help maintaining the product stability and level of quality.
